...
export const useCashStore = defineStore('cash', () => {
  ...
  const receivingTable = () => {
    ...
    axios.get(url, {
      ...
      if (res.data !== undefined) {
        ...
        rowData.value = rowDataAll.value.filter(item => [0].includes(item.is_archive));
      }
    });
  }
  const instruments = (params) => {
    ...
    buttons.innerHTML = `
    <a href="/course-city/view?id=${params.id}" target="_blank" title="Просмотр" >
    ...
    <a href="/course-city/update?id=${params.id}" target="_blank" title="Редактировать">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
        <path d="M471.6 21.7c-21.9-21.9-57.3-21.9-79.2 0L362.3 51.7l97.9 97.9 30.1-30.1c21.9-21.9 21.9-57.3 0-79.2L471.6 21.7zm-299.2 220c-6.1 6.1-10.8 13.6-13.5 21.9l-29.6 88.8c-2.9 8.6-.6 18.1 5.8 24.6s15.9 8.7 24.6 5.8l88.8-29.6c8.2-2.7 15.7-7.4 21.9-13.5L437.7 172.3 339.7 74.3 172.4 241.7zM96 64C43 64 0 107 0 160V416c0 53 43 96 96 96H352c53 0 96-43 96-96V320c0-17.7-14.3-32-32-32s-32 14.3-32 32v96c0 17.7-14.3 32-32 32H96c-17.7 0-32-14.3-32-32V160c0-17.7 14.3-32 32-32h96c17.7 0 32-14.3 32-32s-14.3-32-32-32H96z"/>
      </svg>
    </a>
    <a href="/course-city/delete?id=${params.id}"  target="_blank" title="Удалить" data-method="POST">
    ...
    for (let item of res.value) {
      if (item.city && par.data.city) {
        if (item.city.code === par.data.city.code) {
          if ((par.data.buyCurrency.code === buyCashCode && item.buyCurrency.code === buyCashCode || par.data.buyCurrency.code === sellCashCode && item.buyCurrency.code === sellCashCode) || (par.data.sellCurrency.code === buyCashCode && item.sellCurrency.code === buyCashCode || par.data.sellCurrency.code === sellCashCode && item.sellCurrency.code === sellCashCode)) {
            if ((par.data.sellCurrency.code === buyCurrencyCode && item.sellCurrency.code === sellCurrencyCode || par.data.sellCurrency.code === sellCurrencyCode && item.sellCurrency.code === buyCurrencyCode) || (par.data.buyCurrency.code === buyCurrencyCode && item.buyCurrency.code === sellCurrencyCode || par.data.buyCurrency.code === sellCurrencyCode && item.buyCurrency.code === buyCurrencyCode)) {
